...
const HomePage = () => {
  ...
  const onUploadFile = async (analysisConfig) => {
    ...
    try {
      ...
      setIsLoading(false);
      await loadHistory();
      strapi.notification.success(formatMessage({id: getTrad("manage-catalog-aes.import.success")}));
    } catch (e) {
      setIsLoading(false);
      await loadHistory();
      strapi.notification.error(formatMessage({id: getTrad("manage-catalog-aes.import.error")}));
