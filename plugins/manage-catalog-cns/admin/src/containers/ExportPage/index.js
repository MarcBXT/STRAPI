  ...
  const downloadCatalog  = async () => {
    ...
    try {
      axios.get(
        ...
        download(
          response.data,
          'Catalogue_CNS_'+ moment().toISOString()+'.xlsx' ,
          CONTENT_TYPE.XLSX
        );
        setIsLoading(false);
        strapi.notification.success(formatMessage({id: getTrad("manage-catalog-cns.export.success")}));
      });
    } catch (e) {
      setIsLoading(false);
      console.log(e);
      strapi.notification.error(formatMessage({id: getTrad("manage-catalog-cns.export.error")}));
